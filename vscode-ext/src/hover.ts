...
const HOVER_DOCS: Record<RuleId, string> = {
  ...
  GET_ONLINE_PLAYERS: [
    '**Folia Checker** — `GET_ONLINE_PLAYERS` _(警告)_',
    '',
    '**問題の可能性:**',
    '`getOnlinePlayers()` の結果に対してエンティティ操作を行うと、',
    '各プレイヤーが異なるリージョンスレッドに属するため Folia でスレッド違反になります。',
    '',
    '**修正方法:**',
    '```java',
    '// 各プレイヤーのスケジューラーで操作する',
    ...
    '        p.sendMessage("Hello!");',
    '        p.setHealth(20.0);  // リージョンスレッド内で安全',
    '    }, null);',
    '}',
    '```',
    '',
    '_メッセージ送信のみの場合はスレッド安全です。エンティティ状態の変更時にのみ注意が必要です。_',
  ].join('\n'),

  CANCEL_ALL_TASKS: [
    '**Folia Checker** — `CANCEL_ALL_TASKS`',
    '',
    '**問題の説明:**',
    '`cancelTasks(plugin)` は BukkitScheduler のメソッドで、Folia では使用できません。',
    'Folia では ScheduledTask オブジェクトを個別にキャンセルする必要があります。',
    '',
    '**修正方法:**',
    '```java',
    '// ScheduledTask をリストで管理',
    'private final List<ScheduledTask> tasks = new ArrayList<>();',
    '',
    '// タスク登録時に追加',
    'tasks.add(Bukkit.getGlobalRegionScheduler().runAtFixedRate(...));',
    '',
    '// onDisable でまとめてキャンセル',
    '@Override',
